""" work with paths or files (ragu fork)

Based on UsergeTeam/Userge-Plugins plugins/misc/pathlib/__main__.py
Changes for ravixbabu901/ragu:
- Removed `rarfile` dependency (it often breaks / needs unrar).
- Added `.rar` unpack support via `7z` (install `p7zip-full` in Docker image).
- `.rar` packinfo (listing members) is limited: we show a hint instead of crashing.
"""
...
def _is_rar(file_path: str) -> bool:
...
class _BaseLib:
    ...
    def percentage(self) -> int:
        """ Returns percentage """
        # avoid ZeroDivision
        return int(round((self._current / self._total) * 100, 2)) if self._total else 0
...
class PackLib(_BaseLib):
    ...
    def _unpack_zip_or_tar(self, file_names: List[str]) -> None:
        """Unpack zip/tar members, honoring file_names chunking."""
    ...
    def _unpack_rar_7z(self) -> None:
        """Unpack a .rar using 7z (extract all)."""
        if not _has_7z():
            self._output = "`7z not found. Install p7zip-full in your image.`"
            self._finish()
            return
        try:
            os.makedirs(self._final_file_path, exist_ok=True)
            # Extract all (-y assume Yes, -o set output dir)
            cmd = ["7z", "x", "-y", f"-o{self._final_file_path}", self._file_path]
            proc = subprocess.run(
                cmd,
            ...
            if proc.returncode != 0:
                self._output = f"`7z extract failed (code {proc.returncode})`:\n<code>{proc.stdout[-3500:]}</code>"
            else:
                # We can't easily count members without parsing 7z output
                self._current = self._total or 1
    ...
    def unpack_path(self) -> None:
        """ UNPACK file path """
        # rar via 7z (extract all)
        if _is_rar(self._file_path):
            dir_name = splitext(basename(self._file_path))[0]
            self._final_file_path = join(
                config.Dynamic.DOWN_PATH, dir_name.replace('.', '_'))
            self._total = 1
            pool.submit_thread(self._unpack_rar_7z)
            return

        # zip/tar: chunked member extraction
        chunked_file_names = []
        temp_file_names = []
        temp_size = 0
        ...
        for f_n_s in chunked_file_names:
            pool.submit_thread(self._unpack_zip_or_tar, f_n_s)

    def get_info(self) -> Sequence[Tuple[str, int]]:
        """ Returns PACK info for zip/tar only.
        For rar, return empty and let caller handle.
        """
        if is_zipfile(self._file_path):
            with ZipFile(self._file_path) as z_f:
                return tuple((z_.filename, z_.file_size) for z_ in z_f.infolist())
        if is_tarfile(self._file_path):
    ...
    def is_supported(file_path: str) -> bool:
        ...
        return is_zipfile(file_path) or is_tarfile(file_path) or _is_rar(file_path)
...
class SCLib(_BaseLib):
    ...
    def percentage(self) -> int:
        """ Returns percentage """
        return int(round((self._cmp_size / self._file_size) * 100, 2)) if self._file_size else 0

    @property
    def speed(self) -> float:
        """ Returns speed """
        denom = (time() - self._s_time)
        return int(round(self._cmp_size / denom, 2)) if denom > 0 else 0
        ...
        for p_s in sorted(path_.iterdir(), key=lambda a: sort_file_name_key(a.name)):
            if p_s.is_file():
                if str(p_s).endswith((".mp3", ".flac", ".wav", ".m4a")):
                    ...
                    files += '📹'
                elif str(p_s).endswith((".zip", ".tar", ".tar.gz", ".rar", ".7z")):
                    files += '🗜'
...
async def packinfo_(message: Message) -> None:
    ...
    if _is_rar(file_path):
        if not _has_7z():
            await message.err("`7z not found. Install p7zip-full.`", show_help=False)
        else:
            await message.err("RAR packinfo not implemented. Use: `7z l <file.rar>`", show_help=False)
        return
